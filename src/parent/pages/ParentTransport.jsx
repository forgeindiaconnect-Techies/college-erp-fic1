...
const ParentTransport = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      let activeParent = DEFAULT_PARENT;
      if (session) {
        activeParent = JSON.parse(session);
      } else {
      ...
      try {
        const studentId = activeParent.childId;
        
        // Fetch from the standard student collection
        ...
        const myNotifs = allNotifs.filter(n => 
          (n.targetRole === 'All' || n.targetRole === 'Parent') &&
          (n.routeId === 'All' || n.routeId === (activeRouteId || 'Pending'))
        );
        setNotificationsList(myNotifs);

      } catch (err) {
        console.error("Failed to load child's transport details", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleLogComplaint = async (e) => {
    ...
    try {
      const res = await createTransportComplaint({
        studentId: childDetails.id || childDetails.rollNo,
        name: childDetails.name,
        busNumber: childDetails.busNumber || 'TN01AB1234',
        routeId: childDetails.busRoute,
  ...
  const isTransportUser = childDetails?.transportRequired?.toLowerCase() === 'yes' || !!childDetails?.busRoute;
      ...
      <div className="parent-dashboard animate-fade-in" style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%)', borderRadius: '24px', padding: '3rem', color: 'white', marginBottom: '2.5rem', boxShadow: '0 20px 40px -15px rgba(79, 70, 229, 0.5)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}>
              ...
              <h1 style={{ fontSize: '1.8rem', fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>Child Transport Tracking</h1>
            </div>
            <p style={{ fontSize: '0.95rem', opacity: 0.9, margin: '0 0 0 3.5rem', maxWidth: '600px' }}>Monitor {childDetails.name}'s daily bus route, fee status, and receive real-time notifications about their journey.</p>
          </div>
        </div>
        ...
        {!isTransportUser ? (
          <div className="glass-card" style={{ padding: '4rem', textAlign: 'center', maxWidth: '600px', margin: '0 auto', borderRadius: '24px' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', lineHeight: '1.6' }}>Your child is currently not registered for the college bus facility. If you believe this is an error or wish to apply, please contact the Transport Administration.</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
            {/* Action Bar */}
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <button className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px' }}>
                <Bus size={18} /> View Bus Details
              </button>
              <button className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px' }}>
                <Phone size={18} /> Contact Driver
              </button>
              <button className="btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px' }}>
                <CheckCircle2 size={18} /> View Fee Status
              </button>
              <button className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', borderRadius: '12px', background: '#ef4444', border: 'none' }} onClick={() => setShowComplaintModal(true)}>
            ...
            <div className="glass-card" style={{ borderRadius: '24px', padding: '2.5rem', border: '1px solid rgba(79, 70, 229, 0.1)', boxShadow: '0 10px 40px -10px rgba(0,0,0,0.05)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                <div>
                  <h2 style={{ fontSize: '1.6rem', color: 'var(--text-main)', margin: 0, fontWeight: 800, display: 'flex', alignItems: 'center', gap: '10px' }}>
                    {childDetails.name}'s Bus Details
                  </h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div style={{ fontSize: '1.3rem', color: 'var(--text-main)', fontWeight: 700 }}>{childDetails.busRoute || 'Pending'}</div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div style={{ fontSize: '1.3rem', color: 'var(--text-main)', fontWeight: 700 }}>{childDetails.pickupPoint || 'Pending'}</div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div style={{ fontSize: '1.5rem', color: '#10b981', fontWeight: 900 }}>{childDetails.busNumber || 'Pending'}</div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                  ...
                  <div style={{ fontSize: '1.1rem', color: 'var(--text-main)', fontWeight: 700 }}>
                    Pickup: 7:30 AM<br/>Drop: 5:00 PM
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginTop: '1.5rem' }}>
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '1.2rem', color: 'var(--text-main)', fontWeight: 700 }}>{childDetails.driverName || 'Pending'}</div>
                  </div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '1.2rem', color: 'var(--text-main)', fontWeight: 700 }}>{childDetails.driverContact || 'Pending'}</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: '1.5rem', background: 'linear-gradient(to right, rgba(79,70,229,0.05), rgba(59,130,246,0.05))', padding: '1.5rem 2rem', borderRadius: '16px', border: '1px solid rgba(79, 70, 229, 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Total Amount: ₹{childDetails.transportFeeAmount || 15000}</div>
                  </div>
                </div>
                <div style={{ padding: '10px 24px', borderRadius: '12px', fontWeight: 800, fontSize: '1.1rem', background: childDetails.transportFeeStatus === 'paid' || childDetails.transportFeeStatus?.toLowerCase() === 'paid' ? 'rgba(16,185,129,0.1)' : 'rgba(245,158,11,0.1)', color: childDetails.transportFeeStatus === 'paid' || childDetails.transportFeeStatus?.toLowerCase() === 'paid' ? '#10b981' : '#f59e0b', border: `1px solid ${childDetails.transportFeeStatus === 'paid' || childDetails.transportFeeStatus?.toLowerCase() === 'paid' ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'}` }}>
                  {(childDetails.transportFeeStatus || 'Pending').toUpperCase()}
                </div>
              </div>
            </div>
            ...
            <div className="glass-card" style={{ padding: '2rem', borderRadius: '24px', marginTop: '2rem' }}>
              ...
              {complaintsList.length === 0 ? (
                <div style={{ padding: '3rem', textAlign: 'center', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px dashed var(--border-color)' }}>
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', margin: '8px 0 0' }}>If you are facing any issues with the bus service (delays, driver behavior), click the button above to notify the transport admin.</p>
                </div>
              ) : (
          ...
          <div className="modal-content" style={{ background: 'var(--bg-secondary)', borderRadius: '24px', padding: '2.5rem', width: '90%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', border: '1px solid var(--border-color)', position: 'relative', zIndex: 10000, display: 'block', opacity: 1, visibility: 'visible' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleLogComplaint}>
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <textarea 
                  required
                  placeholder="e.g., The bus did not stop at my child's pickup point today."
