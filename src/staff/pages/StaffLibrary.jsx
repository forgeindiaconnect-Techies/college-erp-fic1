...
import { Book, Search, Library, PlusCircle, CheckCircle, AlertTriangle, BookOpen, Clock, Calendar, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="animate-fade-in p-6">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[var(--text-main)] flex items-center gap-3">
            <Library className="text-purple-500" size={32} /> Staff Library Portal
          </h1>
          <p className="text-[var(--text-muted)] mt-2">Search the library catalog and request research materials.</p>
        </div>
        <button 
      ...
      <div className="flex gap-4 border-b border-[var(--border-color)] mb-8">
        <button 
          className={`pb-3 px-4 font-semibold text-sm transition-colors border-b-2 ${activeTab === 'catalog' ? 'border-purple-500 text-[var(--text-main)]' : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
          onClick={() => setActiveTab('catalog')}
        >
          Library Catalog
        </button>
        <button 
          className={`pb-3 px-4 font-semibold text-sm transition-colors border-b-2 ${activeTab === 'issued' ? 'border-purple-500 text-[var(--text-main)]' : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
      ...
      {!loading && activeTab === 'catalog' && (
        <div className="space-y-6">
          ...
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-2xl overflow-x-auto overflow-y-hidden shadow-sm">
                  ...
                  <tr key={book._id} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-main)] transition-colors">
                    <td className="p-4 font-medium text-[var(--text-main)]">
                      <div className="flex items-center gap-3">
                        <BookOpen size={16} className="text-purple-500" /> {book.title}
                      </div>
                    </td>
              ...
              <tbody>
                {transactions.map((tx) => (
                  <tr key={tx._id} className="border-b border-[var(--border-color)] hover:bg-[var(--bg-main)] transition-colors">
                    <td className="p-4 font-medium text-[var(--text-main)]">
                      <div className="flex items-center gap-3">
                        <BookOpen size={16} className="text-purple-500" /> {tx.bookId?.title || 'Unknown Book'}
                      </div>
                    </td>
